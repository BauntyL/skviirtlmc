...
export async function registerRoutes(
  ...
  const pluginHandler = async (req: any, res: any) => {
    ...
    console.log('[PLUGIN] Request received:', {
      method: req.method,
      url: req.url,
      hasBody: !!req.body,
    ...
    if (!apiKey || apiKey !== validKey) {
        console.log('[PLUGIN] Auth failed:', { apiKey, validKey });
    ...
    try {
      ...
      if (!action || action === 'sync') {
          ...
          try {
            ...
            console.error('[PLUGIN] Error updating server stats:', error.message);
          }
          ...
          try {
            if (Array.isArray(players)) {
              for (const p of players) {
                  try {
                    // Clan Auto-Create
                    if (p.clan && typeof p.clan === 'string') {
                    ...
                    if (user) {
                        ...
                        await db.update(users).set({
                            ...
                            clan: (p.clan && typeof p.clan === 'string') ? p.clan.replace(/[\[\]]/g, "") : null,
                            rank: p.rank,
                    ...
                    console.error('[PLUGIN] Error processing player:', p.name, playerError.message);
                  }
              }
            }
          } catch (error: any) {
            console.error('[PLUGIN] Error updating players:', error.message);
          }
          ...
          try {
            if (Array.isArray(clansList)) {
              const incomingClanNames = clansList
                  .map(c => (c.name && typeof c.name === 'string') ? c.name.replace(/[\[\]]/g, "") : "")
                  .filter(name => name !== "");
                  ...
                  for (const storedClan of allStoredClans) {
                      if (!incomingClanNames.includes(storedClan.name)) {
                          try {
                            ...
                            console.error('[PLUGIN] Error deleting clan:', storedClan.name, deleteError.message);
                          }
                      }
                  }
              }
              ...
              for (const c of clansList) {
                  try {
                    const clanName = (c.name && typeof c.name === 'string') ? c.name.replace(/[\[\]]/g, "") : "";
                    ...
                    console.error('[PLUGIN] Error processing clan:', c.name, clanError.message);
                  }
              }
            }
          } catch (error: any) {
            console.error('[PLUGIN] Error updating clans:', error.message);
          }
      ...
      return res.status(400).json({ message: "Unknown action" });
        ...
        console.error('[PLUGIN] Request body:', JSON.stringify(body).substring(0, 500));
